...
class BPParams:
    ...
    k_per_sig: int = 5
    eps_dom: float = 0.05
    depot_idx: int = -1                   # -1 表示不尝试回仓
    label_params: Optional[LabelSettingParams] = None  # ?????? label-setting
...
class LabelSettingGenerator:
    ...
    # ---------- 输出目录规范化 ----------
    def _prepare_label_outdir(self, label_params: LabelSettingParams, base_outdir: str) -> Optional[str]:
        outdir = getattr(label_params, "outdir", None)
        if not outdir:
            label_params.outdir = None
            return None
        # 相对于 BP 的 outdir 进行拼接（若 label outdir 非绝对路径）
        if not os.path.isabs(outdir) and base_outdir:
    ...
    # ---------- 日志器（只添加一个文件 handler） ----------
    def _prepare_label_logger(
        self,
        label_params: LabelSettingParams,
        outdir: Optional[str],
        fallback: logging.Logger,
    ) -> logging.Logger:
        logger = label_params.logger or fallback

        # 默认：写文件；可用 label_params.log_to_file=False 关闭
        log_to_file = getattr(label_params, "log_to_file", True)
        log_name    = getattr(label_params, "log_name", "label_setting.log")
        log_level   = getattr(label_params, "log_level", logging.INFO)

        if log_to_file and outdir:
            logfile = os.path.join(outdir, log_name)
            abs_logfile = os.path.abspath(logfile)

            def _same_file(h: logging.FileHandler, path: str) -> bool:
                try:
                    return (
                        isinstance(h, logging.FileHandler) and
                        getattr(h, "baseFilename", None) and
                        os.path.exists(h.baseFilename) and
                        os.path.exists(path) and
                        os.path.samefile(h.baseFilename, path)
                    )
                except Exception:
                    # 退化到大小写无关 + 绝对路径比较
                    return os.path.normcase(os.path.abspath(getattr(h, "baseFilename", ""))) == os.path.normcase(path)

            has_handler = any(_same_file(h, abs_logfile) for h in logger.handlers)
            if not has_handler:
            ...
            # 避免传播到父 logger 造成重复打印
            logger.propagate = False

        # 不强制修改 logger 的 level（尊重外部），若需要文件级别用 handler 设置
    ...
    def _maybe_dump_labels(self, labels: Sequence[Any], iteration: int) -> None:
        ...
        dump_csv      = getattr(self.label_params, "dump_csv", True)
        labels_name   = getattr(self.label_params, "labels_csv_name", "labels.csv")
        dump_top_k    = getattr(self.label_params, "dump_top_k", 50)

        if not dump_csv:
            return

        path = os.path.join(outdir, labels_name)
        exists = os.path.exists(path)

        # 以 red_cost 为主键（无 red_cost 回退到 cost）
        top_labels = sorted(
            labels, key=lambda L: getattr(L, "red_cost", getattr(L, "cost", 0.0))
        )[:dump_top_k]
        ...
        with open(path, "a", newline="", encoding="utf-8") as f:
            ...
            if not exists:
                writer.writerow([
                    ...
                    "path", "served_count",
                    "cost", "red_cost",
                    "time", "energy", "latest_departure"
                ])
            for rank, lab in enumerate(top_labels, 1):
                writer.writerow([
                    iteration,
                    rank,
    ...
    def generate(self, duals: Dict[int, float], budget: int, lambda_route: float) -> List[Column]:
        ...
        self.label_logger.info(
            "Pricing: generated %d labels, %d negative-rc candidates, return %d.",
            len(labels),
            len(candidates),
            len(picked),
        )
