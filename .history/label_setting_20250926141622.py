...
class LabelSettingParams:
    ...
    lambda_route: float = 100                         # 每条路径一次性惩罚项（与 RMP 一致）
    seed: Optional[int] = 42                          # 无对偶时的随机扰动、
    outdir: str = "LS_logs"
...
def feasibility_pruning(
    ...
    # 需要可回仓时：检查从当前 node 回 depot 的能量与时间窗是否可行
    if require_return:
        if L.node != depot_idx:
            back_fly = problem.flight_time(L.node, depot_idx)
            if L.energy + 1e-9 < back_fly:
                return True, "cannot_return_energy"
            depot = problem.nodes[depot_idx]
            t_back_arr = L.time + problem.nodes[L.node].service + back_fly
            if t_back_arr > depot.tw[1] + 1e-9:
                return True, "cannot_return_tw"
    return False, ""
