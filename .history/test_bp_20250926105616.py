...
def setup_label_logger(outdir: str, name: str = "LABEL_SETTING") -> logging.Logger:
    os.makedirs(outdir, exist_ok=True)
    logger = logging.getLogger(name)
    logger.setLevel(logging.DEBUG)
...
if __name__ == "__main__":
    ...
    # ★ 新增：label_setting 日志目录与 logger
    LABEL_OUTDIR = os.path.join("bp_logs", "label_setting")
    label_logger = setup_label_logger(LABEL_OUTDIR)
    ...
    # labels = label_setting(prob, params=label_params)
    # logger.info("LabelSetting generated %d labels", len(labels))

    # 4) 运行 BP
    logger.info("Starting Branch-and-Price...")
    bp = BPSolver(problem=prob, params=bp_params, logger=None)
