...
class LabelSettingParams:
    ...
    # 新增：signature 策略配置（可选值示例：'node_served_timebin'(默认)、'node_timebin'、
    # 'served_timebin'、'covered_hash'、'spatial'、'none'）
    signature_strategy: str = "node_served_timebin"
    # 用于 spatial 策略（按距离分桶）的桶大小（同样也可以由外部覆盖）
    signature_dist_bucket: float = 100.0
...
# 新增：signature 工厂，支持多种分组策略
def make_signature_fn(nodes: List[Any], params: LabelSettingParams):
    """
    返回一个函数 f(Label) -> tuple 作为 signature。
    支持策略：
      - node_served_timebin (默认): (node, served_count, time_bin)
      - node_timebin               : (node, time_bin)
      - served_timebin             : (served_count, time_bin)
      - covered_hash               : (len(covered_set), hash(covered_set)) （粗粒度）
      - spatial                    : (distance_bin, served_count)（需要节点坐标 x,y）
      - none                       : ('all', 0) 把所有标签放同一桶（禁用分组）
    """
    strategy = getattr(params, "signature_strategy", "node_served_timebin")
    time_bucket = getattr(params, "time_bucket", 30.0)
    depot_idx = getattr(params, "depot_idx", 0)
    dist_bucket = getattr(params, "signature_dist_bucket", 100.0)

    depot = None
    if 0 <= depot_idx < len(nodes):
        depot = nodes[depot_idx]

    def node_served_timebin(L: Label):
        tbin = int(L.time // time_bucket) if time_bucket and time_bucket > 0 else int(L.time)
        return (L.node, L.served_count, tbin)

    def node_timebin(L: Label):
        tbin = int(L.time // time_bucket) if time_bucket and time_bucket > 0 else int(L.time)
        return (L.node, tbin)

    def served_timebin(L: Label):
        tbin = int(L.time // time_bucket) if time_bucket and time_bucket > 0 else int(L.time)
        return (L.served_count, tbin)

    def covered_hash(L: Label):
        # 长度 + 内容哈希作为签名（粗粒度避免爆炸）
        return (len(L.covered_set), hash(L.covered_set))

    def spatial(L: Label):
        # 以当前节点到仓库距离分桶，fallback 到 node_served_timebin
        try:
            if depot is None:
                return node_served_timebin(L)
            nj = nodes[L.node]
            dx = getattr(nj, "x", None)
            dy = getattr(nj, "y", None)
            if dx is None or dy is None or getattr(depot, "x", None) is None or getattr(depot, "y", None) is None:
                return node_served_timebin(L)
            dist = math.hypot(nj.x - depot.x, nj.y - depot.y)
            dbin = int(dist // dist_bucket) if dist_bucket and dist_bucket > 0 else int(dist)
            return (dbin, L.served_count)
        except Exception:
            return node_served_timebin(L)

    def none_sig(L: Label):
        return ("all", 0)

    mapping = {
        "node_served_timebin": node_served_timebin,
        "node_timebin": node_timebin,
        "served_timebin": served_timebin,
...
def label_setting(
    ...
    # 使用组件化 signature（由外部策略控制）
    signature = make_signature_fn(nodes, params if params is not None else LabelSettingParams())
    ...
    for s in problem.customers:
        ...
        sig = signature(init)
        dom, reason = try_dominance(init, make_signature_fn[sig])
        if dom:
            if logger: logger.debug(f"[INIT-DOM] {init.path} -> {reason}")
            continue

        _kbest_insert(make_signature_fn, sig, init, K_per_sig)
        stack.append(init)
