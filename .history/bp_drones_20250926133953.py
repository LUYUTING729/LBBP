...
class LabelSettingGenerator:
    ...
    def __init__(self, problem: Problem, params: BPParams, logger: logging.Logger):
        ...
        self.params = params
        self.logger = logger
        self._seen_keys: Set[tuple] = set()   # ?????? path tuple ?? key
        self._gen_call = 0                    # ?????? id ??
        self.label_params = self._resolve_label_params(params, logger)
        self.label_outdir = self._prepare_label_outdir(self.label_params, params.outdir)
        self.label_logger = self._prepare_label_logger(self.label_params, logger)

    def _resolve_label_params(self, bp_params: BPParams, default_logger: logging.Logger) -> LabelSettingParams:
        if bp_params.label_params is not None:
            params = replace(bp_params.label_params)
        else:
            params = LabelSettingParams(
                max_len=bp_params.max_label_len,
                depot_idx=bp_params.depot_idx,
                logger=default_logger,
                K_per_sig=bp_params.k_per_sig,
                eps=bp_params.eps_dom,
            )
    ...
    def _prepare_label_outdir(self, label_params: LabelSettingParams, base_outdir: str) -> Optional[str]:
        outdir = label_params.outdir
        if not outdir:
            return None
        base = base_outdir or ""
        if not os.path.isabs(outdir):
            outdir = os.path.join(base, outdir) if base else outdir
        os.makedirs(outdir, exist_ok=True)
        label_params.outdir = outdir
        return outdir

    def _prepare_label_logger(
        self, label_params: LabelSettingParams, fallback: logging.Logger
    ) -> logging.Logger:
        logger = label_params.logger or fallback
        if  LabelSettingParams.outdir:
            logfile = os.path.join(LabelSettingParams.outdir, label_params.log_name)
            abs_logfile = os.path.abspath(logfile)
            for handler in logger.handlers:
                if isinstance(handler, logging.FileHandler) and getattr(handler, "baseFilename", None) == abs_logfile:
                    break
            else:
                fh = logging.FileHandler(abs_logfile, mode="w", encoding="utf-8")
                fh.setLevel(label_params.log_level)
    ...
    def _maybe_dump_labels(self, labels: Sequence[Any], iteration: int) -> None:
        if not self.label_params.dump_csv or not self.label_outdir:
            return
        path = os.path.join(self.label_outdir, self.label_params.labels_csv_name)
        exists = os.path.exists(path)
        top_labels = sorted(
            labels, key=lambda L: getattr(L, "red_cost", getattr(L, "cost", 0.0))
        )[: self.label_params.dump_top_k]
        with open(path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if not exists:
                writer.writerow(["iteration", "rank", "path", "cost", "red_cost", "served_count", "latest_departure"])
