...
def build_toy_instance():
    ...
    return Problem(
        ...
        K_max=1,
        D_max=1,
        tT={(i.id, j.id): 1.0 for i in nodes for j in nodes if i.id != j.id},
        tD={(i.id, j.id): 1.0 for i in nodes for j in nodes if i.id != j.id},
        cT=1.0,
        cD=1.0,
        F=0.0,
        U_d=10.0,
        Q_d=5.0,
        e={n.id: 0 for n in nodes},
        l={n.id: 999 for n in nodes},
        g={n.id: n.demand for n in nodes},
    )
