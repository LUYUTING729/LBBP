...
class BPSolver:
    ...
    def _column_generation_loop(self, node: BranchNode) -> tuple[float, float, int]:
        ...
        while iter_in_node < self.params.max_iterations:
            ...
            for b in range(self.params.pricing_batch):
                new_cols = self.cg.generate(used_duals, budget=50, lambda_route=self.params.rmp_params.lambda_route)
                if not new_cols:
                    break
                # 应用分支界限（对子节点刚加入的新列，也要立刻设定 LB/UB）
                node.rmp.add_columns(new_cols)
                self._apply_fixed_bounds(node)
                # 统计 rc
                rc_min_round = min(rc_min_round, min(c.meta.get("rc", 0.0) for c in new_cols))
                new_cols_total += len(new_cols)
                round_cols += len(new_cols)
    ...
    def _record(self, iteration: int, node: BranchNode, obj: float, rc_min: float, new_cols: int) -> None:
        ...
        gap = max(0.0, (obj - self.global_best_obj) / max(1.0, abs(self.global_best_obj))) if self.global_best_obj < float("inf") else float("inf")
        st = BPStatus(
            iteration=iteration,
            node_id=node.id,
            obj_value=obj,
        ...
        self.logger.info("[Node %d] obj=%.6f, new_cols=%d, rc_min=%.6g, elapsed=%.2fs, gap=%s",
                         node.id, obj, new_cols, rc_min, elapsed, f"{gap:.4%}" if gap < float("inf") else "NA")

        # 每轮快照（包含当前 RMP 的结构化文件，由 RMP 自行写出）
        if self.params.dump_every_iter:
            snap_dir = os.path.join(self.params.outdir, f"iter_{iteration}_node_{node.id}")
            os.makedirs(snap_dir, exist_ok=True)
            # 拷贝/重写 RMP 输出（RMP 内部已经写 columns/selected/duals/solution 至其 outdir）
            # 这里只记一个 summary json
            summary = {
                "iteration": iteration,
                "node_id": node.id,
