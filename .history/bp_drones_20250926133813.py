...
class LabelSettingGenerator:
    ...
    def _prepare_label_logger(
        self, label_params: LabelSettingParams, outdir: Optional[str], fallback: logging.Logger
    ) -> logging.Logger:
        logger = label_params.logger or fallback
        if label_params.log_to_file and outdir:
            logfile = os.path.join(outdir, label_params.log_name)
