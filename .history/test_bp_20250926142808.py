...
def gen_customers(n=20, seed=42) -> List[Node]:
    ...
    for cid in range(1, n + 1):
        ...
        # 时间窗：适度宽松但有差异
        start = random.uniform(0, 600)
        width = random.uniform(600, 1200)
...
if __name__ == "__main__":
    ...
    # 1) 构造问题：1个仓库 + 20 客户
    depot = Node(0, demand=0.0, tw=(0, 1800), service=0.0, x=0.0, y=0.0)
    cust_nodes = gen_customers(n=40, seed=24)
    nodes = [depot] + cust_nodes
    customers = [n.id for n in cust_nodes]

    drone = DroneSpec(capacity=80.0, endurance=600.0, speed=5)
